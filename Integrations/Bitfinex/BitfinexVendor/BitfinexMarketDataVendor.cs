...
namespace BitfinexVendor
{
    internal class BitfinexMarketDataVendor : BitfinexInternalVendor
    {
        ...
        public override ConnectionResult Connect(ConnectRequestParameters parameters)
        {
            ...
            // Assets
            string[][][] assetLabels = this.HandleApiResponse(() => this.Api.PublicRestApiV2.GetConfigs<string[][][]>("map", "currency", "label", cancellation), out string error);
            ...
            // Symbols
            var symbols = this.HandleApiResponse(() => this.Api.RestApiV1.GetSymbolDetails(cancellation), out error);
            ...
            // Derivatives
            string[][] derivatives = this.HandleApiResponse(() => this.Api.PublicRestApiV2.GetConfigs<string[][]>("list", "pair", "futures", cancellation), out error);
            ...
            // Derivatives status
            var statuses = this.HandleApiResponse(() => this.Api.PublicRestApiV2.GetDerivativesStatus(cancellation), out error);
            ...
            // Tickers
            var tickers = this.HandleApiResponse(() => this.Api.PublicRestApiV2.GetTickers(cancellation), out error);
            ...
            if (requestParameters.Period.BasePeriod == BasePeriod.Tick)
            {
                ...
                while (fromUnix < currentToUnix)
                {
                    var trades = this.HandleApiResponse(
                        () => this.Api.PublicRestApiV2.GetTrades(symbol, fromUnix, currentToUnix, cancellation), out string _, true, true);
            ...
            else
            {
                ...
                while (fromUnix < currentToUnix)
                {
                    var candles = this.HandleApiResponse(
                        () => this.Api.PublicRestApiV2.GetCandles(symbol, timeFrame, fromUnix, currentToUnix, cancellation), out string _, true, true);
        ...
        private IEnumerable<BitfinexDerivativeStatus> LoadDerivativeStatus(string symbol, long fromUnix, long toUnix, CancellationToken token)
        {
            ...
            do
            {
                ...
                statuses = this.HandleApiResponse(() => this.Api.PublicRestApiV2.GetDerivativesStatusHistory(symbol, request, token), out _, true, true);
                if (statuses == null)
                    yield break;
        ...
        {
            var statuses = this.HandleApiResponse(() => this.Api.PublicRestApiV2.GetDerivativesStatus(this.GlobalCancellation), out string error);
