...
namespace BitfinexVendor
{
    internal class BitfinexTradingVendor : BitfinexMarketDataVendor
    {
        ...
        {
            // Base margin info
            var marginInfo = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetMarginInfo<BitfinexMarginInfo>(BitfinexMarginKey.BASE, token), out string error);
            ...
            var symbolsMarginInfo = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetMarginInfo<BitfinexMarginInfo[]>(BitfinexMarginKey.ALL_SYMBOLS, token), out error);
            ...
            // User info
            this.Context.UserInfo = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetUserInfo(token), out error);

            if (!string.IsNullOrEmpty(error))
                throw new InvalidOperationException(error);

            // Account summary
            this.Context.AccountSummary = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetAccountSummary(token), out error);
        ...
        {
            var wallets = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetWallets(token), out string error);
            ...
            return wallets.Select(this.CreateAssetBalances).ToList();
        }
        ...
        {
            var orders = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetActiveOrders(token), out string error);
        ...
        {
            var positions = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetPositions(token), out string error);
        ...
        public override IList<MessageTrade> GetTrades(TradesHistoryRequestParameters parameters)
        {
            ...
            do
            {
                ...
                trades = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetUserTrades(request, token), out _, true);
                if (trades == null)
                    break;
        ...
        private TradingOperationResult MakeTradingOperation(Func<Task<BitfinexOrderResponse>> tradingOperation)
        {
            TradingOperationResult result = default;

            try
            {
                var orderResponse = this.HandleApiResponse(() => tradingOperation.Invoke(), out string error);

                if (!string.IsNullOrEmpty(error))
                    return result = TradingOperationResult.CreateError(error);

                if (orderResponse == null)
                    return result = TradingOperationResult.CreateError("Order response is null");

                if (orderResponse.Status != BitfinexStatus.SUCCESS)
                    return result = TradingOperationResult.CreateError(orderResponse.Text);

                return result = TradingOperationResult.CreateSuccess(orderResponse.Orders.FirstOrDefault()?.Id.ToString());
            }
            catch (Exception ex)
            {
                return result = TradingOperationResult.CreateError(ex.GetFullMessageRecursive());
            }
            finally
            {
        ...
        public override Report GenerateReport(ReportRequestParameters parameters) => parameters.ReportType.Id switch
        {
            BitfinexVendor.REPORT_ORDERS_HISTORY => this.GenerateOrdersHistoryReport(parameters),
            _=> null
        };
        ...
        private IEnumerable<BitfinexOrder> LoadOrders(DateTime from, DateTime to, CancellationToken cancellation)
        {
            ...
            do
            {
                ...
                orders = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetOrdersHistory(request, cancellation), out _, true);
                if (orders == null)
                    break;
        ...
        {
            var tickers = this.HandleApiResponse(() => this.Api.PublicRestApiV2.GetTickers(this.GlobalCancellation), out string error);

            if (!string.IsNullOrEmpty(error))
            {
                Core.Instance.Loggers.Log(error, LoggingLevel.Error, BitfinexVendor.VENDOR_NAME);
                return;
            }

            this.Context.UpdateTickers(tickers);

            foreach (var message in this.Context.Wallets.Values.Select(this.CreateAssetBalances))
                this.PushMessage(message);
        }

        private void UpdateMarginInfoAction()
        {
            var marginInfo = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetMarginInfo<BitfinexMarginInfo>(BitfinexMarginKey.BASE, this.GlobalCancellation), out string error);
        ...
        {
            var marginInfo = this.HandleApiResponse(() => this.Api.PrivateRestApiV2.GetMarginInfo<BitfinexMarginInfo[]>(BitfinexMarginKey.ALL_SYMBOLS, this.GlobalCancellation), out string error);
        ...
        private void PrivateWebSocketApiOnNewData(object sender, BitfinexEventArgs e)
        {
            ...
            if (e.OrderUpdate != null)
            {
                ...
                if (orderHistory.OrderTypeId != OrderType.TrailingStop)
                {
                    ...
                    this.PushMessage(DealTicketGenerator.CreateTradingDealTicket(orderHistory, BitfinexVendor.VENDOR_NAME));
                }
